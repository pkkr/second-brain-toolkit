...
run_install() {
    ...
    HOME="$home" XDG_BIN_HOME="$home/bin" \
        "$ROOT/install.sh" --no-agent-links "$@" >/dev/null
}

SYMLINK_HOME="$TEMP_ROOT/symlink"
mkdir -p "$SYMLINK_HOME"
run_install "$SYMLINK_HOME" --data-mode symlink
...
run_install "$MOVE_HOME" --data-mode move
...
run_install "$CONVERT_HOME" --data-mode move
...
if run_install "$CONFLICT_HOME" --data-mode move 2>/dev/null; then
...
echo "install.sh data-location tests passed"
